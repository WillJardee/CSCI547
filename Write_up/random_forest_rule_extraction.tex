...
\end{center}
After studying the resulting boards that were returned, one can convince themselves that there is some underlying structure. However, the visualization on boards of this sort doesn't aid in learning an abstract rule like ``if three `x' in a row $\leftrightarrow$ positive." There does seem to be some promise in this area with boards like like $2$ in figure~\ref{fig:ttt0-1-5000} and number of possible `x' or `o' rows in the positive and negative examples, relatively.
